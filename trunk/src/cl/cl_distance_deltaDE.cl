formulaOut CalculateDistance(float4 point, sClFractal *fractal)
{
	...
	else
	{
		...
		float r1 = Fractal(point + (float4)
		{ delta, 0.0, 0.0, 0.0}, fractal).distance;
		dr.x = fabs(r1 - r) / delta;
		float r2 = Fractal(point + (float4)
		{ 0.0, delta, 0.0, 0.0}, fractal).distance;
		dr.y = fabs(r2 - r) / delta;
		float r3 = Fractal(point + (float4)
		{ 0.0, 0.0, delta, 0.0}, fractal).distance;
